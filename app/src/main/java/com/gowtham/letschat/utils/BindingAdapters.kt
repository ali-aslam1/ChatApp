...
if (list.size<=2){
    ...
    fun getLastMsgTxt(msg: Message) : String{
        return when(msg.type){
            ...
            "image" -> {
                msg.imageMessage?.imageType.toString().replaceFirstChar {
    ...
    fun groupMsgStatus(txtStatus: TextView, message: GroupMessage) {
        ...
        val statusList=message.status.toMutableList()
        val myStatus=statusList.firstOrNull() ?: 0
        if (message.from==preference.getUid())
            statusList.removeAt(0)
        val deliveried=statusList.any{ it==2 || it==3 }  //if anyone has seen the message
        val seen=statusList.isNotEmpty() && statusList.all{ it==3 }  //all members seen the messge
    ...
    fun groupLastMessage(txtView: TextView, group: GroupWithMessages) {
        ...
        if (messages.isEmpty()){
            ...
            val msg="Created by ${group.group.members?.firstOrNull { it.id==createdBy }?.localName ?: "Unknown"}"
            txtView.text=msg
        }
        else{
            val message=messages.last()
            val localName=group.group.members?.firstOrNull { it.id==message.from }?.localName ?: "Unknown"
    ...
    fun setChatUserName(txtView: TextView, users: Array<ChatUser>, message: GroupMessage) {
       val messageOwner= users.firstOrNull { message.from==it.id }
       txtView.text=messageOwner?.localName ?: "Unknown"
    }
    ...
    fun showUserIdIfNotLocalSaved(
        ...
        val messageOwner= users.firstOrNull { message.from==it.id }
        txtView.text=messageOwner?.user?.userName ?: "Unknown"
        if (messageOwner?.locallySaved == true)
            txtView.gone()
        else
            txtView.show()
    }
