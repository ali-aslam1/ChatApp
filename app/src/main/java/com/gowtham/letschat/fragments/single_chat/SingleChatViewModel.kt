...
constructor(
    ...
    private fun attachStatusListener(userId: String) {
        if (userId.isBlank()) return
        statusListener?.let {
            statusRef?.removeEventListener(it)
        }
        statusRef = database.getReference("Users/$userId")
    ...
    fun setChatUser(chatUser: ChatUser) {
        ...
        if (toUser != chatUser.id) {
            toUser = chatUser.id
            attachStatusListener(toUser)
        }
        if (fromUser != null) {
            setSeenAllMessage()
        }
    ...
    fun sendMessage(message: Message) {
        if (toUser.isBlank() || fromUser.isNullOrBlank()) return
