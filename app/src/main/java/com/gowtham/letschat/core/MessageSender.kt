            ...
            msgCollection.document(doc)
                ...
                .addOnSuccessListener {
                    msgCollection.document(doc).collection("messages").document(message.createdAt.toString()).set(
                        ...
                        message.chatUserId=chatUserId
                        message.status=4
                        LogMessage.v("Message sender Failed ${error.message}")
                        Timber.e("MessageSender: Failed - ${error.message}")
                        listener.onFailed(message)
                    }
                }.addOnFailureListener { error ->
                    message.chatUserId=chatUserId
                    message.status=4
                    LogMessage.v("Message sender Failed chat_members update ${error.message}")
                    Timber.e("MessageSender: Failed to update chat_members - ${error.message}")
                    listener.onFailed(message)
                }
        } catch (e: Exception) {
            Timber.e("MessageSender: Exception - ${e.message}")
            e.printStackTrace()
        }
    }
